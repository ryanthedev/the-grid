...
actor NotificationStore {
    ...
    // Reads persisted state from disk. Called once at server startup.
    ...
    func load() {
        ...
        do {
            ...
            // - Append any byID keys missing from orderedIDs (shouldn't happen, but be safe)
    ...
    // Forces an immediate write if dirty. Called on server shutdown.
    func flush() {
        if isDirty {
            persistNow()
        }
    ...
    // Multiple markDirty calls are absorbed by the debounce window.
    @discardableResult

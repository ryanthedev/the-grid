...
completion(Response(id: request.id, result: AnyCodable(["success": true])))
    ...
    func registerGridHandlers(
        ...
        // ============================================================
        // NOTIFY RPCs -- delegate to @notify commands via router
        // ============================================================
        ...
        register(method: "grid.notify.push") { request, completion in
            ...
            // Quote values so the router's tokenizer preserves multi-word strings
            var cmd = "@notify push \"\(title)\""
            if let body = request.params?["body"]?.value as? String, !body.isEmpty {
                cmd += " --body \"\(body)\""
            }
            ...
            if let action = request.params?["action"]?.value as? String, !action.isEmpty {
                cmd += " --action \"\(action)\""
            }
            dispatchAndRespond(request, commandString: cmd, completion: completion)
        }
        ...
        register(method: "grid.notify.list") { request, completion in
            ...
            // list returns a JSON string in result.message; use the structured handler
            Task {
                jlog("grid.rpc.dispatch", data: ["method": request.method, "cmd": cmd])
                let result = await router.dispatch(cmd)
                if result.success {
                    // message is a JSON array string; try to parse it back into an array
                    if let data = result.message.data(using: .utf8),
                       let array = try? JSONSerialization.jsonObject(with: data) {
                        completion(Response(id: request.id, result: AnyCodable(["notifications": array])))
                    } else {
                        completion(Response(id: request.id, result: AnyCodable(["notifications": []])))
                    }
                } else {
                    completion(Response(id: request.id, error: ErrorInfo(code: -32000, message: result.message)))
                }
            }
        }
        ...
        register(method: "grid.notify.count") { request, completion in
            let cmd = "@notify count"
            // count returns a numeric string in result.message; forward as-is
            dispatchAndRespond(request, commandString: cmd, completion: completion)
        }

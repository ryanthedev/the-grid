...
class WindowManipulator {
    ...
    private let scriptingAddition: ScriptingAdditionClient

    init(connectionID: Int32, logger: Logger) {
        self.connectionID = connectionID
        self.logger = logger
        self.scriptingAddition = ScriptingAdditionClient(logger: logger)
    }
    ...
    func moveWindowToSpace(windowID: UInt32, spaceID: UInt64) -> Bool {
        ...
        // Determine method based on macOS version and SA availability
        let needsWorkaround = needsCompatibilityWorkaround()

        if needsWorkaround {
            // macOS 12.7+, 13.6+, 14.5+, 15+ - try SA first, then compatibility
            logger.info("🔀 Modern macOS detected - checking for Scripting Addition")

            // Check if SA is available
            if scriptingAddition.isAvailable() {
                logger.info("✓ Scripting Addition available - using privileged method")

                // Use SA to move window
                let success = scriptingAddition.moveWindowToSpace(windowID: windowID, spaceID: spaceID)

                if !success {
                    logger.error("❌ SA move API failed", metadata: [
                        "windowID": "\(windowID)",
                        "spaceID": "\(spaceID)"
                    ])
                    return false
                ...
                if verified {
                    logger.info("✓ Window moved successfully via SA", metadata: [
                        "windowID": "\(windowID)",
                    ...
                    logger.error("❌ SA move verification failed", metadata: [
                        "windowID": "\(windowID)",
                ...
                // SA not available - warn user
                logger.warning("⚠️  Scripting Addition not available on macOS \(ProcessInfo.processInfo.operatingSystemVersion.majorVersion)+", metadata: [
                    "socketPath": "/tmp/grid-sa_\(NSUserName()).socket"
                ])
                logger.warning("⚠️  Window move to space functionality requires SA installation")
                logger.warning("⚠️  See GridSA/README.md for installation instructions")

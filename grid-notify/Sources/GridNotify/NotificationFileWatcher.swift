...
class NotificationFileWatcher {
    ...
    // Callback invoked on MainActor after a notification is added.
    // Set by the caller to trigger view model refresh.
    ...
    func start() {
        guard !config.path.isEmpty else {
            jlog("notify.watcher.skip", msg: "no path configured", data: [:])
            return
        }
        ...
        jlog("notify.watcher.start", data: ["path": config.path])
        queue.async {
            self.openAndWatch()
        }
    }
    ...
    private func openAndWatch() {
        ...
        guard newFD >= 0 else {
            jlog("err.notify.watcher.open", data: [
                "path": config.path,
                "errno": errno
            ])
        ...
        guard fstat(fd, &statBuf) == 0 else {
            jlog("err.notify.watcher.fstat", data: [
                "path": config.path,
                "errno": errno
            ])
            tearDown()
        ...
        jlog("notify.watcher.opened", data: [
            "path": config.path,
            "fifo": isFIFO
        ])
    }
    ...
    private func handleReadable(isFIFO: Bool) {
        ...
        while true {
            ...
            if bytesRead > 0 {
                ...
                jlog("err.notify.watcher.read", data: ["errno": errno])
                break
            }
        }
        ...
        if gotEOF {
            ...
            if isFIFO {
                ...
                jlog("notify.watcher.pipe.eof", data: ["path": config.path])
                tearDown()
    ...
    private func handleFSEvent() {
        ...
        if eventMask.contains(.delete) || eventMask.contains(.rename) {
            ...
            jlog("notify.watcher.rotate", data: ["path": config.path])
            tearDown()
    ...
    private func processLine(_ line: String) {
        ...
        do {
            ...
            jlog("err.notify.watcher.parse", msg: "invalid line JSON", data: [
                "err": "\(error)"
            ])
        }
    }
    ...
    private func parseLineAction(_ dict: [String: String]?) -> GridNotificationAction? {
        ...
        default:
            jlog("warn.notify.watcher.action", msg: "unknown action type", data: ["type": type])
            return nil
        }
    }
